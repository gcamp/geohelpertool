...
describe('parseMultiFormat', () => {
  ...
  describe('polyline unescape option', () => {
    it('should pass unescape option to polyline parser', () => {
      ...
      const resultWithUnescape = parseMultiFormat(polylineWithBackslashes, { unescape: true })
      const resultWithoutUnescape = parseMultiFormat(polylineWithBackslashes, { unescape: false })
      ...
      expect(resultWithUnescape.success === resultWithoutUnescape.success ||
             resultWithUnescape.error !== resultWithoutUnescape.error).toBe(true)
    })
  })
  ...
  describe('error handling', () => {
    ...
    it('should provide comprehensive error when all formats fail', () => {
      const unparsableInput = 'this is completely unparsable data that matches no format'
  ...
  describe('format-specific edge cases', () => {
    ...
    it('should handle WKT with unusual formatting', () => {
      const weirdFormattedWKT = '  point  (  -73.9857    40.7484  )  '
      
      const result = parseMultiFormat(weirdFormattedWKT)
      expect(result.success).toBe(true)
      // This might be detected as coordinates due to the numeric content
      // Since the coordinate parser is last and more permissive
      expect([LayerType.WKT, LayerType.COORDINATES]).toContain(result.format)
    })
  })
})
...
describe('detectAndParseLayer', () => {
  ...
  it('should handle different input formats consistently', () => {
    const inputs = [
      ...
      { data: '{"type":"Feature","geometry":{"type":"Point","coordinates":[-73.9857,40.7484]},"properties":{}}', expected: LayerType.GEOJSON }
    ]
...
describe('format detection edge cases', () => {
  it('should handle very long input strings', () => {
    ...
    const longCoordList = Array.from({ length: 1000 }, (_, i) => `${40 + i * 0.001},${-73 + i * 0.001}`).join(',')
  ...
  it('should handle malformed JSON gracefully', () => {
    const malformedJson = '{"type":"Feature","geometry":{"type":"Point","coordinates":[-73.9857,40.7484]}'  // Missing closing brace
